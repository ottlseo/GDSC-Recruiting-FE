...
const ApplicationArea = styled.div`
  ...
  padding-left: 10%;
`
const CoreApplyPage = (infoState, showing) => { 
  ...
  const [info, setInfo] = useState({
    ...
    tel: infoState.tel,
    major: infoState.major,
    studentNum: infoState.studentNum,
    isCore: infoState.isCore
  });

  const { userId, name, email, tel, major, studentNum, isCore } = info;
  const generateInfoRequestDto = (userId, name, email, tel, major, studentNum, isCore) => {
    info.userId = userId;
    info.name = name;
    info.email = email;
...
*/  var temporaryInfo = { //우선 임시로 할당하는 정보
  ...
  const [inputs, setInputs] = useState({
    ...
    //generateInfoRequestDto();
    return ( 
      show && <ApplicationArea>
        <p>안녕하세요, {infoState.name}님.
          GDSC Ewha Core team에 지원해주셔서 감사합니다. 아래의 정보를 입력해주세요.</p>
